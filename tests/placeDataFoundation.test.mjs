...
import { boundingBox, haversineDistanceKm, isInsideBounds, parseCoordinate } from '../src/lib/places/geo.js'
import {
  mergeUniquePlaces,
  normalizeProviderPlace,
  placeMatchConfidence,
} from '../src/lib/places/normalizePlace.js'
import { OlaPlacesProvider } from '../src/lib/places/olaPlacesProvider.js'
import { createPlaceSearchService } from '../src/lib/places/placeSearchService.js'
import { FilePlaceRepository } from '../src/lib/places/filePlaceRepository.js'
import { WikimediaImageProvider } from '../src/lib/places/wikimediaImageProvider.js'
import { AnakinImageProvider, extractPageImage, safePublicUrl } from '../src/lib/places/anakinImageProvider.js'
import { imageCandidateFromWikimedia } from '../src/lib/places/imageCandidate.js'
import { presentLivePlace } from '../src/lib/livePlaces.js'
import { FallbackPlaceRepository } from '../src/lib/places/filePlaceRepository.js'
import {
  RequestValidationError,
  createPlacesApiHandler,
  parseNearbyQuery,
  parseBoundsQuery,
  parseSearchQuery,
} from '../src/lib/places/request.js'
import { findCategory, providerTypesForCategory } from '../src/lib/places/taxonomy.js'
...
test('API handler returns a stable envelope and private cache policy', async () => {
  const handler = createPlacesApiHandler({
    parse: parseSearchQuery,
    service: async () => ({ places: [place()], meta: { providerStatus: 'not-needed' } }),
  })
  const res = mockRes()
  await handler({ method: 'GET', query: { q: 'Flurys' } }, res)
...
test('API handler exposes validation errors but hides service failures', async () => {
  ...
  const handler = createPlacesApiHandler({ parse: parseSearchQuery, service })

  const validation = mockRes()
  await handler({ method: 'GET', query: { q: '' } }, validation)
  ...
  console.error = () => {}
  try {
    await handler({ method: 'GET', query: { q: 'coffee' } }, failure)
  } finally {
    console.error = originalError
  }
  assert.equal(failure.statusCode, 503)
  assert.equal(JSON.stringify(failure.body).includes('DATABASE_URL'), false)
})
